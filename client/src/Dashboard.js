...
const Profile = ({ profile }) => (
  <div className='profile bg-light'>
    ...
    <div>
      ...
      <p>{profile.mobile}</p>
      <p>{profile.skill}</p>
      <Link to={`/indprofile/${profile.fullname}/${profile.email}/${profile.mobile}/${profile._id}`} className='btn btn-primary'>
        View Profile
      </Link>
    </div>
  </div>
  ...
  return (
    <div>
      ...
      <section className='container'>
        ...
        <p className='lead'>
          <i className='fab fa-connectdevelop'></i> Browse and connect with Employers
        </p>
        <div className='profiles'>
          {data.length > 0 ? (
            data.map((profile, index) => <Profile key={index} profile={profile} />)
          ) : (
